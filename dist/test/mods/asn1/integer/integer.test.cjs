...
function hexToInteger(hex) {
    ...
    return integer.Integer.fromDER(new binary.Binary(buffer)).value;
}
uvu.test("Read", () => tslib.__awaiter(void 0, void 0, void 0, function* () {
    ...
    assert.assert(hexToInteger("02 01 64") === BigInt(100));
    assert.assert(hexToInteger("02 01 9C") === BigInt(-100));
    assert.assert(hexToInteger("02 02 00 FF") == BigInt(255));
    assert.assert(hexToInteger("02 01 80") == BigInt(-128));
    assert.assert(hexToInteger("02 05 80 00 00 00 01") === BigInt("-549755813887"));
}));
uvu.test.run();
